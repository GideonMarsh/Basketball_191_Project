...
making new project
1. create project
2. copy over main, src, incude, and images (and maybe others if added later)
3. add those files to project
4. add libSoil to project->build options->linker settings and move it to the top
*/
#pragma once  // only search once
...
// windows handling - to work when multiple windows are open
// part of windows programming
...
// The following variables makes speed consistent regardless of framerate
float FPS;
...
BOOL CreateGLWindow(char* title, int width, int height, int bits, bool fullscreenflag)
{
	...
	// sets window to be in upper left and size of screen
...
{
    Scene->winMsg(hWnd, uMsg, wParam, lParam);      // pass windows parameters into GLScene
					...
					int			nCmdShow)			// Window Show State
{
		...
		else										// If There Are No Messages
		{
			...
			if (keys[VK_F1])						// Is F1 Being Pressed?
			{
               ...
               if(Scene->helpFlag == true && keys[81])
               {
                ...
                //User presses spacebar to confirm to shoot
                //Need to add code to allow user to shoot once at the end of the court
  ...
  //                    Ply->action = "stand";
                ...
                //Player will need to press spacebar to shoot when at the shootingScene
